...
def cal_forgetting(acc_all, task_num):
    a = np.array(acc_all)
    result = []
...
def main():
    ...
    log_root = '../log/Tiny-Imagenet/%s/phases_%d/seed_%d-%s-b64-step'%(args.data_name, args.task_num, args.seed, s)
    ...
    for current_task in range(args.task_num+1):
        ...
        model.model = torch.load(filename)
        ...
        for i in range(current_task+1):
            ...
            accuracy = model._test(test_loader)
    ...
    forgetting = cal_forgetting(acc_all, args.task_num)
    logging.info(30 * '#')
