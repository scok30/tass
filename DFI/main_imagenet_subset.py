...
def imagenet_subset_sal_detection(config, dataset_root, save_root):
...
# 由 sal 计算 edge 图并保存
def cal_edge_map(lowlevel_root, img_form='.JPEG'):
    ...
    for i, folder_name in enumerate(folder_list):
        ...
        for i, filename in enumerate(tqdm(sal_img_list, desc=f'Cal edge map')):
            ...
            # im_size = tuple(sal_map.shape[:2])
            # sal_map = cv2.resize(sal_map, (256, 256), interpolation=cv2.INTER_LINEAR)
            edge_map = LaplacianEdgeDection(sal_map)
            # edge_map = cv2.resize(edge_map, (im_size[1], im_size[0]))
...
if __name__ == '__main__':
    ...
    save_root = './lowlevel_data/imagenet100'
    # Sal detection
    imagenet_subset_sal_detection(config,
                                dataset_root=dataset_root,
                                save_root=save_root)
    # Cal edge map from sal map
    cal_edge_map(lowlevel_root=save_root, img_form='.JPEG')