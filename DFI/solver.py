...
class Solver(object):
    ...
    def test(self, test_mode=0):
        ...
        test_mode_name = ['Edge detection', 'Sal detection', 'Skel detection', 'All']
        EPSILON = 1e-8
        img_num = len(self.data_loader)
        for i, data_batch in enumerate(tqdm(self.data_loader, desc=test_mode_name[test_mode])):
            images, name, im_size = data_batch['image'], data_batch['name'][0], np.array([tensor.item() for tensor in data_batch['size']])
            if test_mode == 0: # edge task
                ...
                cv2.imwrite(os.path.join(self.config.test_fold, name[:-len(self.config.img_form)] + '_' + mode_name[test_mode] + self.config.img_form), multi_fuse)
                ...
                with torch.no_grad():
                    ...
                    multi_fuse = 255 * pred
                    # cv2.imwrite(os.path.join(self.config.test_fold, name[:-len(self.config.img_form)] + '_' + mode_name[test_mode] + self.config.img_form), multi_fuse)
                    cv2.imwrite(os.path.join(self.config.test_fold, name[:-len(self.config.img_form)]  + self.config.img_form), multi_fuse)
